...
if __name__ == "__main__":
    ...
    # load hopenet model to initialize parameters (trained for non-occluded images)
    saved_state_dict = torch.load('models/hopenet_robust_alpha1.pkl')
    ...
    # occluded_path = natsort.natsorted(glob.glob(
    #    'D:\datasets\Data_Ocluded_300WLP_FULL_25_2\*.jpg'), reverse=False)  # occluded images
    # image_path = natsort.natsorted(
    #    glob.glob('D:\datasets\300W-LP_Full\*.jpg'), reverse=False)  # non-occluded images

    # small datasets for training
    occluded_path = natsort.natsorted(glob.glob(
        'datasets\occluded\*.jpg'), reverse=False)  # occluded images
    image_path = natsort.natsorted(
        glob.glob('datasets\original\*.jpg'), reverse=False)  # non-occluded images

    # Pose annotations
    d_mat1 = natsort.natsorted(
        glob.glob('datasets/300W_LP/AFW/*.mat'), reverse=False)
    d_mat2 = natsort.natsorted(
        glob.glob('datasets/300W_LP/HELEN/*.mat'), reverse=False)
    d_mat3 = natsort.natsorted(
        glob.glob('datasets/300W_LP/IBUG/*.mat'), reverse=False)
    d_mat4 = natsort.natsorted(
        glob.glob('datasets/300W_LP/LFPW/*.mat'), reverse=False)
    ...
    transformations = transforms.Compose([transforms.Resize(240), transforms.RandomCrop(
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels, cont_labels, xi) in enumerate(train_loader):
            ...
            # Bin labels translation
            #label_t1 = Variable(labels[:, 0]).cuda(gpu)
            #label_t2 = Variable(labels[:, 1]).cuda(gpu)
            #label_t3 = Variable(labels[:, 2]).cuda(gpu)
            ...
            # Compute translation continuous values through expected value
            ...
            if (i+1) % 100 == 0:
                ...
                print('Average Angle Error (degrees): Yaw: %.4f, Pitch %.4f, Roll %.4f' % (
                    y_err, p_err, r_err))
        # Save models at numbered epochs.
        if epoch % 2 == 0 and epoch < num_epochs:
            print('Taking snapshot...')
            torch.save(model.state_dict(), 'Latent' + '_model' +
